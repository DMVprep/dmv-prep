        ...
        <div className="space-y-4 mb-8">
          ...
          <div className="rounded-2xl border-2 border-green-300 bg-green-50 p-6 shadow-sm">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="font-bold text-gray-900 text-xl">Practice Mode</h2>
                <p className="text-sm text-gray-600 mt-1 mb-1">Learn at your own pace with instant feedback and explanations after every question.</p>
                <p className="text-xs text-green-700 font-medium mb-4">✓ Free · No time limit · Perfect for learning</p>
                ...
                <button onClick={() => startTest("PRACTICE")} className="px-6 py-3 bg-green-500 text-white rounded-xl text-sm font-bold hover:bg-green-600 transition-colors shadow-sm">
                  Start Practice →
                </button>
              </div>
            </div>
          ...
          {/* Premium modes — visually secondary when not premium */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Exam Simulation */}
            <div className={cn("rounded-xl border p-4", IS_PREMIUM ? "border-blue-200 bg-blue-50" : "border-gray-200 bg-gray-50")}>
              <div className="flex items-center gap-3 mb-2">
                <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0", IS_PREMIUM ? "bg-blue-500" : "bg-gray-300")}>
                  {IS_PREMIUM ? <Target className="w-4 h-4 text-white" /> : <Lock className="w-4 h-4 text-white" />}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold text-gray-900 text-sm">Exam Simulation</h2>
                    {!IS_PREMIUM && <span className="px-1.5 py-0.5 bg-yellow-100 text-yellow-800 text-[10px] font-bold rounded-full">PREMIUM</span>}
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-500 mb-3">
                Real {state.name} DMV format. Timed, no hints, pass/fail result.
              </p>
              {IS_PREMIUM ? (
                <button onClick={() => startTest("EXAM_SIMULATION")} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-xs font-semibold hover:bg-blue-600 transition-colors w-full">
                  Start Exam Simulation
                </button>
              ) : (
                <Link href="/pricing" className="flex items-center justify-center gap-1 px-4 py-2 bg-gray-200 text-gray-600 rounded-lg text-xs font-semibold hover:bg-yellow-100 hover:text-yellow-800 transition-colors w-full">
                  Unlock with Premium <ArrowRight className="w-3 h-3" />
                </Link>
              )}
            </div>

            {/* Weak Areas */}
            <div className={cn("rounded-xl border p-4", IS_PREMIUM ? "border-orange-200 bg-orange-50" : "border-gray-200 bg-gray-50")}>
              <div className="flex items-center gap-3 mb-2">
                <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0", IS_PREMIUM ? "bg-orange-500" : "bg-gray-300")}>
                  {IS_PREMIUM ? <AlertTriangle className="w-4 h-4 text-white" /> : <Lock className="w-4 h-4 text-white" />}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold text-gray-900 text-sm">Weak Areas</h2>
                    {!IS_PREMIUM && <span className="px-1.5 py-0.5 bg-yellow-100 text-yellow-800 text-[10px] font-bold rounded-full">PREMIUM</span>}
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-500 mb-3">
                Questions you previously got wrong. Targeted improvement.
              </p>
              {IS_PREMIUM ? (
                <button onClick={() => startTest("WEAK_AREAS")} className="px-4 py-2 bg-orange-500 text-white rounded-lg text-xs font-semibold hover:bg-orange-600 transition-colors w-full">
                  Study Weak Areas
                </button>
              ) : (
                <Link href="/pricing" className="flex items-center justify-center gap-1 px-4 py-2 bg-gray-200 text-gray-600 rounded-lg text-xs font-semibold hover:bg-yellow-100 hover:text-yellow-800 transition-colors w-full">
